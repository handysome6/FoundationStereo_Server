...
def process_stereo_images(args):
    """
    主处理函数，处理立体视觉图像并生成视差图和点云
    ...
    # 保存处理后的输入图像
    imageio.imwrite(f'{args.out_dir}/img0.png', img0)
    imageio.imwrite(f'{args.out_dir}/img1.png', img1)
    ...
    if args.get_pc:
        ...
        # 创建掩码来标识不合格的点
        invalid_mask = ~((np.asarray(pcd.points)[:,2]>0) & (np.asarray(pcd.points)[:,2]<=args.z_far))
        # 将不合格点的坐标设置为(0,0,0)
...
def process_stereo_images_cli():
    ...
    # 读取cfg.yaml文件
    cfg_path = 'pretrained_models/23-51-11/cfg.yaml'
...
if __name__ == "__main__":
    # args = OmegaConf.load('server_config.yaml')
    # process_stereo_images(args)

    # 使用命令行参数
    process_stereo_images_cli()